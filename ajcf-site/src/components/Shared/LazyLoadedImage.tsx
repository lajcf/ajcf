...
const LazyLoadedImage = memo((props: PropsWithChildren<LazyLoadedImageProps>) => {
  const [intersectionEntry, setNode] = useIntersectionObserver({});
  const [loadImage, imageRef, onImageLoad] = useImageBlurPlaceholder(props.id, intersectionEntry?.isIntersecting);
  ...
  return (
    <>
      <div
        ...
        style={placeholderContainerStyle(props.dominantColor, props.containerStyle)}
        className={cx("image-placeholder", props.containerClassName)}
      >
        {props.children}
        {loadImage && (
          <img ref={imageRef} src={props.srcLarge} onLoad={onImageLoad} style={props.imageStyle} alt={props.alt} />
        )}
