...
export const addEntitiesToGoogleSheet = async <T extends object>(entities: T[]): Promise<number> => {
  ...
  return bearerClient.integration("google_sheets")
    .auth(process.env.GOOGLE_SHEET_API_BEARER_KEY!)
    .put<GoogleSheetUpddateValuesResponse>(`${process.env.GOOGLE_SHEET_MEMBERS_ID}/values/${entities[0].constructor.name}!A1`,
      {
        query: {
...
export const createGoogleSheet = async (sheetName: string): Promise<string> => {
  const bearerClient = bearer(process.env.BEARER_SECRET_KEY);
  const response = await bearerClient.integration("google_drive")
...
export const fetchGoogleSheet = async (sheetId: string): Promise<FetchGoogleSheetResponse | null> => {
  try {
    const bearerClient = bearer(process.env.BEARER_SECRET_KEY);
    const response = await bearerClient.integration("google_sheets")
      .auth(process.env.GOOGLE_SHEET_API_BEARER_KEY!)
