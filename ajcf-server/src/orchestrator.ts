...
export const upsertCampaign = async (
  campaign: HelloAssoCampaign,
  lookupSheet: FetchGoogleSheetResponse
) => {
  const rowData = lookupSheet.sheets[0].data[0].rowData;
  const campaignSheetInfo = extractEntityFromSheet(
    campaign.id,
    "helloAssoId",
    rowData
  );
  if (!campaignSheetInfo) {
    ...
    await Promise.all(
      payments.map((p) => insertPayment(p, newCampaignSheetId))
    );
    return;
  }
  const campaignSheet = await fetchGoogleSheet(
    campaignSheetInfo["googleSheetId"]
  );
  const payments = await fetchPayments({
    campaignId: campaign.id,
  });
